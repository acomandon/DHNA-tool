...
1. **Data acquisition** (run once, or whenever refreshing inputs):
   ...
   - **`scripts/data_downloader_dewey.py`** (optional) — pulls Dewey rental partner data (Renthub, Dwellsy TotalIQ) as parquet trees into `data/dewey/<partner>/`. Runs from the `dewey` conda env (see the Python sidecar note below) and needs `DEWEY_API_KEY`. Consumed two ways: (a) `scripts/exploratory/rental_coverage_audit.R` for coverage diagnostics; (b) the **rent_buffer rebuild** — `scripts/extract_renthub_ky.py` (dewey-env pyarrow: filters the raw parquet to KY listings → `data/processed/renthub_ky_listings.csv`, since R's `arrow` isn't reliably installed here) then `scripts/build_rent_buffer.R` (sf spatial join + 1-mile buffer + quarterly-median aggregation → `data/prepackaged/renthub/rent_buffer.csv`, the file the production pipeline reads). Run `build_rent_buffer.R` with `WRITE=1` to overwrite the canonical CSV (defaults to a dry-run that validates against the existing file). Refresh cadence: Renthub is fresher than the ACS, so re-aggregating bumps `optional_data$renthub$recent_*` in `R/config.R` without touching NHGIS.
...
| `06_tract_data.R` | HUD AFFH tract burden, NHGIS tract ACS, FMI-bracketed renter counts; the A2 low-income-renter-loss signal (`rank_lir_loss`); collates `ct_data`, joins BG+CT, builds percentile ranks. |
...
- **A2 low-income-renter-loss signal (`rank_lir_loss`)** — a *direct displacement-observed* measure (renter `distress` family) computed in `06_tract_data.R`: the change in the low-income share of the rental stock, 2010→2020, from B25118 (tenure×income). B25118 is tract-only; the 2010 vintage is fetched separately (dataset `acs_5yr_prior_b`, cells `U26`, via `scripts/fetch_b25118_prior.R` / `data_downloader.R`) and moved onto 2020 tracts with a **renter-weighted crosswalk** aggregated from the NHGIS bg2010→bg2020 crosswalk (`wt_renthu`) — this handles tract splits/merges/redraws and GEOID-reuse uniformly (a naive same-GEOID join silently corrupts ~6 Jefferson tracts). Low-income is CPI-aligned bracket cuts (2010 <\$25k / 2020 <\$35k). In `R/risk_matrix.R`: top-quintile loss → **medium** (advanced displacement, ungated, so it bypasses the appreciation/`pop_change` gates that miss thinning areas); middle-band loss + a composite signal → **high** (nascent). Verification trail: `scripts/exploratory/{verify_tract_crosswalk,build_lir_crosswalk,methodology_experiments,classifier_diagnostics}.R`.
- **Area confidence flag (`area_confidence`)** — transparency, **not** an override. Because every classifier input is local-area-pooled (see below), a BG straddling dissimilar sub-areas gets a confident-looking but blended reading. Stage 06 compares each BG's *own* low-income-household share (+ its ACS margin of error, from the `AURTM*` cells the estimate-only pipeline otherwise drops) to the pooled share, and flags `high` / `moderate` / `low` (`low` = focal confidently differs from its pool beyond the MOE). The Shiny app surfaces only the `low` tier — a "local knowledge advised" note on both the affordability results page (concise) and the Area overview (detailed) via the shared `.confidence_note()` helper in `DHNA/R/mod_area_overview.R`. Policy rationale (why it's a flag, not a reclassification): a straddling BG is deliberately classified to its **dominant landscape**, since letting a vulnerable pocket force high-risk would impose affordable-housing requirements that block development in a high-opportunity zone; the flag defers the pocket judgement to a planner with local knowledge. Verification: `scripts/exploratory/{pocket_confidence,pocket_axes}.R`.
- **Project recommenders** live in `R/project_recommender.R`. `recommend_project_rental(...)` is called from `DHNA/R/mod_affordability_rental.R`; `recommend_project_ownership(...)` from `DHNA/R/mod_affordability_ownership.R`. Each module exposes a `recommendation_result()` reactive — computed once per submit and shared by the `risk_assessment_res` and `recommendation` renderUI blocks.
...
- **`DHNA/R/mod_*.R`** — one module per panel: `mod_intro` (UI-only), `mod_housing_need`, `mod_location`, `mod_affordability_rental`, `mod_affordability_ownership`, `mod_area_overview`. The two affordability modules share the same nav-panel `value = "form"` so navigation works regardless of which is inserted.

Flow: **Location** collects project **tenure** (Rental vs Homeownership), size/type, and resolves the focal block group (`bg_id`) from an address or map click; on *next page*, `server.R` inserts the **Affordability levels** panel **matching the tenure choice**; on *See details*, it inserts the **Area overview** panel. The two runtime-inserted panels' module servers are created up front in `server.R` (Shiny binds their outputs lazily, so they render when the UI appears). Convention: modules receive data frames and selection reactives (`bg_id`, `proj_size`) as **arguments**; theming constants and `locality` config are referenced as **globals**. All inputs are CSVs/SHPs in `DHNA/data/` produced by the pipeline.
